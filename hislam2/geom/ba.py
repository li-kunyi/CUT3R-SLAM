...
def JDSA(target, weight, eta, poses, disps, intrinsics, disps_prior, dscales, ii, jj, alpha):
    """
    Perform Joint Depth and Scale Adjustment (JDSA) for SLAM optimization.

    Args:
        target (torch.Tensor): Target image tensor of shape (B, N, 2).
        weight (torch.Tensor): Weight tensor of shape (B, N, 1).
        eta (torch.Tensor): Regularization parameter tensor of shape (ht, wd).
        poses (torch.Tensor): Pose tensor of shape (B, P, 6).
        disps (torch.Tensor): Disparity tensor of shape (B, P, ht, wd).
        intrinsics (torch.Tensor): Camera intrinsics tensor of shape (B, 4).
        disps_prior (torch.Tensor): Prior disparity tensor of shape (M, ht, wd).
        dscales (torch.Tensor): Depth scale tensor of shape (B, hs, ws).
        ii (torch.Tensor): Index tensor for source points of shape (N,).
        jj (torch.Tensor): Index tensor for target points of shape (N,).
        alpha (float): Regularization parameter.

    Returns:
        tuple: A tuple containing:
            - disps (torch.Tensor): Updated disparity tensor of shape (B, P, ht, wd).
            - dscales (torch.Tensor): Updated depth scale tensor of shape (B, hs, ws).
            - dzcov (torch.Tensor): Covariance tensor of the depth updates of shape (B, M, ht, wd).
    """
