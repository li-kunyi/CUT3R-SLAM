...
from util.utils import Log, clone_obj, pose_vec_to_matrix, compute_distance_v2
...
from gaussian.utils.slam_utils import update_pose, get_pose, to_se3_vec, get_loss_normal, get_loss_mapping_rgbd, depth_to_normal, apply_depth_colormap, project2world
...
from util.utils import get_depth_normal, TV_loss, compute_distance_from_depth
...
class GSBackEnd(mp.Process):
    ...
    def pose_refinement(self, pose, gt_img, tstamp, gt_depth=None, confs=None):  
    ...
    def prefilter(self, fliter_window):
        ...
        with torch.no_grad(): 
            ...
            alpha_binary = (alpha <= 0.7).float()

        return alpha_binary
    
    def local_BA(self, BA_window, fx, fy, cx, cy, iters=50):  
        '''
        ...
        i_indices = torch.arange(1, B).cuda()
        j_indices = torch.arange(B - 1).cuda()

        pbar = tqdm(range(iters), desc="local BA")
        for iteration in range(iters):
            ...
            for viewpoint in viewpoints:
                ...
                if existing_ratio > 0.1:
                    ...
                    alpha_weight = alpha_mask.sum() / (alpha_mask.reshape(-1).shape[0])

                    rgb_loss = torch.abs((gt_image - image)[:, alpha_mask[0]]).mean() * alpha_weight
                    depth_loss = torch.abs(1./depth[depth_pixel_mask] - 1./gt_depth[depth_pixel_mask]).mean() * alpha_weight
            ...
            alpha_binary = (alpha_all <= 0.7).float()

            T_w2c = torch.stack(w2c_all, dim=0)  # new pose
            gt_images = torch.stack(gt_image_all, dim=0)
            gt_depths = torch.cat(gt_depth_all, dim=0)

            # project depth with updated pose to get the pointmaps
            # pointmaps = project2world(torch.inverse(T_w2c), gt_depths, fx, fy, cx, cy)

            # for i_th view, project pointmap, depth, color to previsou i-1_th view, compute the error
            # geo_loss, photo_loss = compute_distance_v2(pointmaps, gt_images, gt_depths, T_w2c, i_indices, j_indices, fx, fy, cx, cy)

            loss = (rgb_loss_all + depth_loss_all) / B #+ 10 * geo_loss + 10 * photo_loss
    ...
    def optimization(self, iters, prune=True, optimize_pose=True, current_window=None, densify=True):
        '''jointly optimize both gaussians and camera poses: local BA'''
        ...
        for idx, view in enumerate(viewpoint_stack):
            ...
            opt_params.append({
                        "params": [view.scale],
                        "lr": self.config["opt_params"]["scale_lr"],
                        "name": "scale_{}".format(view.uid)})
            opt_params.append({
                        "params": [view.shift],
                        "lr": self.config["opt_params"]["scale_lr"],
                        "name": "shift_{}".format(view.uid)})
        ...
        for iteration in range(iters):
            ...
            for viewpoint in viewpoints:
                ...
                depth_pixel_mask = torch.logical_and(gt_depth > 0.001, depth > 0.001).view(*depth.shape)
                scaled_gt_depth = gt_depth * torch.exp(viewpoint.scale) + viewpoint.shift
                depth_loss = torch.abs(1./depth[depth_pixel_mask] - 1./scaled_gt_depth[depth_pixel_mask]).mean()
                # depth_loss = torch.abs(depth[depth_pixel_mask] - scaled_gt_depth[depth_pixel_mask]).mean()
                
                tv_loss, _ = TV_loss(depth, image=gt_image.permute(1, 2, 0)[None])
                ...
                gt_depth_all.append(scaled_gt_depth)
                w2c_all.append(rel_w2c)
                gt_image_all.append(gt_image.permute(1, 2, 0))
        
            loss = (rgb_loss_all + 
                    self.lambda_depth * depth_loss_all + 
                    # self.lambda_tv * tv_loss_all + 
                    self.lambda_normal * normal_loss_all +
                    self.lambda_iso * isotropic_loss_all) / N #+ 10 * geo_loss + 10 * photo_loss
            ...
            with torch.no_grad():    
                ...
                if prune:
                    ...
                    # update_gaussian = (iteration % self.mapping_update_every == self.mapping_update_offset)
                    update_gaussian = iteration == self.mapping_update_offset
                    if update_gaussian:
    ...
    def viz(self, viewpoint, dir, idx):
        ...
        gt_image = viewpoint.original_image.cuda()
        image_ = (torch.exp(viewpoint.exposure_a)) * image + viewpoint.exposure_b

        gt_depth = viewpoint.depth.to(dtype=torch.float32, device=image.device)[..., None]
        gt_depth = gt_depth * torch.exp(viewpoint.scale) + viewpoint.shift
    ...
    def data_update(self, h, w):
        ...
        with torch.no_grad():
            ...
            for view in viewpoint_stack:                    
                ...
                gt_depth = view.depth.to(dtype=torch.float32, device=depth.device)[None]
                gt_depth = gt_depth * torch.exp(view.scale) + view.shift

                normal, pointmap = depth_to_normal(view, gt_depth, world_frame=True)
            ...
            return updated_packet, self.current_window
    ...
    def gaussain_update(self, packet):
        with torch.no_grad():
            ...
            scales = packet['scale_updates'][:-1].squeeze(-1).squeeze(-1).cpu()
            ...
            for i, camera_idx in enumerate(camera_ids):
                ...
                s = scales[(camera_idx // 5 == submap_idx)][0]

                self.viewpoints[camera_idx].depth = self.viewpoints[camera_idx].depth * s
            ...
            align_s = packet['scale_updates'].cuda()[indices]
                
            xyz = self.gaussians.get_xyz[gaussian_indices]
            new_xyz = (torch.matmul(align_s * xyz.unsqueeze(1), align_R.transpose(1, 2)) + align_t).squeeze(1)

            scale = self.gaussians.get_scaling[gaussian_indices]
            new_scaling = self.gaussians.scaling_inverse_activation(scale * align_s.squeeze(-1))
            # new_scaling = self.gaussians.scaling_inverse_activation(scale)

            rot = SO3(self.gaussians.get_rotation[gaussian_indices])
            new_rotation = (SO3(poses[:,3:]) * rot).data
            # new_rotation = rot.data
    ...
    def run(self, packet, iterations=100):     
        ...
        pointmaps = packet['pointmaps'].numpy()
        ...
        imgs_ds = F.interpolate(imgs, size=(h1, w1), mode='bilinear', align_corners=False)
        ...
        # add new gaussian per submap, mapping and joint optimization gaussians and poses
        if not self.initialized:
            self.reset()
            self.gaussians.extend_from_pcd_seq(rgb=imgs_ds, 
                                               pointmap=pointmaps, 
                                            #    normal=normals[:, ::self.downsample_ratio, ::self.downsample_ratio], 
                                               normal=None, 
                                               conf=confs, 
                                               submap_idx=0, init=True)
            self.current_window = list(viz_idx)
            self.optimization(iterations, current_window=self.current_window, densify=True)
            self.initialized = True
        else:
            confs = self.prefilter(viz_idx)
            # pointmaps, confs = self.local_BA(viz_idx, self.fx, self.fy, self.cx, self.cy, iters=10)
            pointmaps = F.interpolate(pointmaps.permute(0, 3, 1, 2), size=(h1, w1), mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
            pointmaps = pointmaps.detach().cpu().numpy()
            confs = F.interpolate(confs[None], size=(h1, w1), mode='bilinear', align_corners=False)[0]
            confs = confs.detach().cpu().numpy()

            self.gaussians.extend_from_pcd_seq(rgb=imgs_ds[1:], 
                                               pointmap=pointmaps[1:], 
                                               normal=None, 
                                               conf=confs[1:], 
                                               submap_idx=submap_idx, init=False)
            self.current_window = self.current_window[-6:-1] + list(viz_idx)
            
            self.optimization(iterations, current_window=self.current_window, densify=True)

        return self.data_update(h1, w1)


    def global_BA(self, iteration_total):
        ...
        for iteration in range(iteration_total):
            viewpoints = list(self.viewpoints.values())
            ...
            loss = (rgb_loss_all + 
                    self.lambda_depth * depth_loss_all + 
                    self.lambda_iso * isotropic_loss_all +
                    self.lambda_tv * tv_loss_all +
                    self.lambda_normal * normal_loss_all
                    ) / N
            ...
            with torch.no_grad():  
                if iteration < (iteration_total // 2) and iteration > 0:
                    for idx in range(len(viewspace_point_tensor_acm)):
                        self.gaussians.max_radii2D[visibility_filter_acm[idx]] = torch.max(
                            self.gaussians.max_radii2D[visibility_filter_acm[idx]],
                            radii_acm[idx][visibility_filter_acm[idx]],
                ...
                if self.verbose and iteration % 200 == 0:
                    for view in list(self.viewpoints.values()):
                        os.makedirs(f"{self.output_dir}/rendering_GBA", exist_ok = True)
                        self.viz(view, f"{self.output_dir}/rendering_GBA", iteration)
    ...
    #         loss = rgb_loss + self.lambda_depth * depth_loss + self.lambda_normal * normal_loss #+ self.lambda_iso * isotropic_loss + self.lambda_tv * tv_loss * 0.5
      
    #         loss.backward()

    #         with torch.no_grad():  
    #             if iteration < (iteration_total // 2) and iteration > 1000:
    ...
    #             if self.verbose and iteration % 300 == 0:
    #                 for view in list(self.viewpoints.values()):
    #                     os.makedirs(f"{self.output_dir}/rendering_GBA", exist_ok = True)
    #                     self.viz(view, f"{self.output_dir}/rendering_GBA", iteration)
