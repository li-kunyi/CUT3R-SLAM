...
import geometry_eval_utils as geom_utils
...
def load_7scenes(dataset, W, H, calib):
    ...
    rgb_files = natsorted(list((dataset / "seq-01/color").glob("*.color.png")))[::subsample]
    depth_files = natsorted(list((dataset / "seq-01/depth").glob("*.depth.png")))[::subsample]
    pose_files = natsorted(list((dataset / "seq-01/pose").glob("*.pose.txt")))[::subsample]
...
def depth2point(color, depth, pose_WC, W, H, calib, valid_mask):
    """
    Returns the ground truth trajectory and point cloud in the world coordinate
    """
    fx, fy, cx, cy = calib  # kinect intrinsics
...
if __name__ == "__main__":
    ...
    parser.add_argument("--render_path", default="outputs/7scenes/renders_kf")
    ...
    for a, b in zip(matches[0], matches[1]):
        ...
        gt_c = np.asarray(gt_pcds[int(a)].colors)
        pcd_gt.append(gt_p)
        gt_color_list.append(gt_c)
        ...
        color_path = f'{dataset}/seq-01/color/frame-{a:06d}.color.png'
        color = cv2.imread(color_path)
        color = cv2.cvtColor(color, cv2.COLOR_BGR2RGB)

        psoe_C2W = traj_est_poses[b]
        est_points, est_colors = depth2point(color, depth, psoe_C2W, W, H, calib, valid_mask)
        pcd_est_list.append(est_points)
        color_est_list.append(est_colors)

    pcd_est = np.concatenate(pcd_est_list, axis=0)
    color_est = np.concatenate(color_est_list, axis=0)
    gt_points = np.concatenate(pcd_gt, axis=0)
    gt_colors = np.concatenate(gt_color_list, axis=0)
    
    pcd_est = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(pcd_est))
    pcd_est.colors = o3d.utility.Vector3dVector(color_est)

    # est_pcd_path = f'{args.render_path}/gaussians_viz_20000.ply'
    # pcd_est = o3d.io.read_point_cloud(est_pcd_path)
    ...
    o3d.io.write_point_cloud(f'{args.render_path}/pcd_est_aligned.ply', pcd_est)
    o3d.io.write_point_cloud(f'{args.render_path}/pcd_gt.ply', gt_pcd)
    print(f"Saved aligned estimated point cloud with color to: {args.render_path}/pcd_est_aligned.ply")
    print(f"Saved ground truth point cloud with color to: {args.render_path}/pcd_gt.ply")
    ...
    with open(f'{args.render_path}/3D_eval_results.txt', "w") as f:
