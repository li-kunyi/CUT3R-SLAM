...
for seq in seqs:
    ...
    # run HI-SLAM2
    cmd = f'python demo.py --imagedir {seq}/rgb --gtdepthdir {seq}/depth '
    ...
    if not os.path.exists(f'{out}/{name}/ape.txt') or len(open(f'{out}/{name}/ape.txt').readlines()) < 10:
        os.system(f'evo_ape tum {seq}/traj_w_c.txt {out}/{name}/traj_full.txt -vas --save_results {out}/{name}/evo.zip --no_warnings > {out}/{name}/ape.txt')
